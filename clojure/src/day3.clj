...
(defn enriched-neighbors
  [grid {:keys [number coords] :as _number}]
  (->> coords
       (mapcat point-neighbors)
       set
       (filter (fn [{:keys [x y] :as point}]
                 (= \* (get-in grid [y x]))))
       (mapv (fn [m]
...
(def part2
  ...
  (->> GRID
       (map-indexed parse-row)
       flatten
       (map (partial enriched-neighbors GRID))
       (apply concat)
       (group-by (juxt :x :y))
       (filter (fn [[coords nums]]
                 (= 2 (count nums))))
       (map (fn [[coords entries]]
